...
namespace ERP_Application.Services
{
    public class NoticiaService : INoticiaService
    {
        ...
        private async Task<NewsApiResponseModel> BuscarNoticiasNaNewsApiAsync(int page, int pageSize)
        {
            ...
            request.Headers.UserAgent.ParseAdd("GenialSaudeNewsApp/1.0");
        ...
        private async Task<string> BuscarConteudoCompletoDaNoticiaNaUrlOriginalAsync(NewsApiArticleModel artigo)
        {
            ...
            try
            {
                var html = await BaixarHtmlDaNoticiaOriginalAsync(artigo.Url);
        ...
        private async Task<string> BaixarHtmlDaNoticiaOriginalAsync(string url)
        {
            var httpClient = _httpClientFactory.CreateClient();
        ...
        private static NewsApiErrorResponseModel? DesserializarErroDaNewsApi(string json)
        {
            try
            {
                return JsonSerializer.Deserialize<NewsApiErrorResponseModel>(json, CriarOpcoesDeDesserializacaoDaNewsApi());
            }
            catch (JsonException)
            {
                return null;
